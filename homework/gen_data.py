...
def generate_with_LangChain(input_content):
    system_content = """
...
def dataset_parser(ai_gen_content):
    """
    extract content and summary from ai response data string
    """
    # 分割字符串来找到content和summary的位置
    content_start = ai_gen_content.find('content:"') + len('content:"')
    content_end = ai_gen_content.find('"\nsummary:')
    summary_start = ai_gen_content.find('summary:"') + len('summary:"')
    summary_end = ai_gen_content.rfind('"')

    # 提取并存储content和summary
    content = ai_gen_content[content_start:content_end].strip()
    summary = ai_gen_content[summary_start:summary_end].strip()
...
def main():
    ...
    with open(filename, mode='w', newline='', encoding='utf-8') as file:
        ...
        for raw_content in raw_content_data:
            ...
            ai_message_content = generate_with_LangChain(raw_content)
